...
public class ReportDao extends BaseDao<Object> {
	...
	public List getTotalStatement(String sDate, String eDate, String activity_id) throws DaoLevelException{
	...
	public Map<String, EverydayRecordVo> getEverydayStatement(String sDate,
			String eDate, String activity_id) throws DaoLevelException{
		...
		try {
			...
			String[] weekday = { "", "星期日", "星期一", "星期二", "星期三", "星期四", "星期五",
					"星期六" };
			...
			for (int i = 0; i < etSize + 1; i++) {
				if (i == etSize) {
					...
					q.setResultTransformer(Transformers
							.aliasToBean(DayAmount.class));
					List<DayAmount> amountList = q.list();
					for (DayAmount dayAmount : amountList) {
						EverydayRecordVo vo = everydayRecords.get(dayAmount
								.getTime());
					...
					for (DayCount dayCount : dayCounts) {
						EverydayRecordVo vo = everydayRecords.get(dayCount
								.getTime());
	...
	public List<MerchantExRecordVo> getMerchantTotalByExType(String sDate,
			String eDate, String activity_id, String exType) throws DaoLevelException{
		logger.debug("call getMerchantTotalByExType({}, {}, {},{})",
				new Object[] { sDate, eDate, activity_id, exType });
		StringBuffer sql = new StringBuffer();
		...
		try {
			...
			for (MerchantExRecord vo : vos) {
				sql = new StringBuffer();
				sql.append("select am1.posid as posid, '"
						+ exType
						+ "'  as 'type', (select count(*) from QQMeishiXaction mshi  ");
				...
				q.setResultTransformer(Transformers
						.aliasToBean(PosTypeCountVo.class));
	...
	public DetailExchRecord getDetailExchRecordByTokenAndActivityId(
		...
		logger.debug("call getDetailExchRecordByTokenAndActivityId({}, {})",
				new Object[] { activity_id, token });
		StringBuffer sql = new StringBuffer();
		...
		try {
			...
			q.setResultTransformer(Transformers
					.aliasToBean(DetailExchRecord.class));
	...
	public List<String> getExchangeTypes(String activity_id) throws DaoLevelException{
		logger.debug("call getExchangeTypes({})", activity_id);
		StringBuffer sql = new StringBuffer();
	...
	public List<MerchantVo> getMerchantVos(String activity_id)
		...
		logger.debug("call getMerchantVos({})", activity_id);
		StringBuffer sql = new StringBuffer();
		sql.append("select m1.id as 'id', m1.merchant_name as 'shopName'  " +
				" from Merchant m1 where  m1.id in (select a1.merchant_id  from Activitymerchant a1  where a1.activity_id=:activity_id" +
				") order by m1.id asc");
		logger.trace(sql.toString());
		try {
			SQLQuery q = getSession().createSQLQuery(sql.toString());
			...
			q.addScalar("id", Hibernate.STRING);
			q.addScalar("shopName", Hibernate.STRING);
			q.setResultTransformer(Transformers.aliasToBean(MerchantVo.class));
			List<MerchantVo> vos = q.list();
			if(vos==null || vos.size()==0){
				return null;
			}
			return vos;
	...
	public int getDetailExchRecordCount(String sDate, String eDate,
			String activity_id, String merchantId) throws DaoLevelException{
		
		
		logger.debug("call getDetailExchRecordCount({}, {}, {}, {})", new Object[]{sDate, eDate, activity_id, merchantId});
		if(merchantId==null || merchantId.trim().length()==0){
			return 0;
		}
		StringBuffer sql = new StringBuffer();
		sql.append("select count(*) as 'tsCount'  from QQMeishiXaction mshi ");
		sql = concatLimitTime(sql, "mshi.ts", sDate, eDate);
		sql.append(" and mshi.xactResultCode = '0'  and mshi.posid in ");
		if(merchantId.equals("all")){
			sql.append(" ( select a1.posid from Merchant m1,Activitymerchant a1 where m1.id=a1.merchant_id and a1.activity_id='"+activity_id+"') ");
		}else{
			sql.append(" ( select a1.posid from Merchant m1,Activitymerchant a1 where m1.id=a1.merchant_id and m1.id='"+merchantId+"') ");
		}
		logger.trace(sql.toString());
		try {
			SQLQuery q = getSession().createSQLQuery(sql.toString());
			q.addScalar("tsCount", Hibernate.INTEGER);
	...
	public List<DetailExchRecord> getDetailExchRecords(String sDate,
		...
		logger.debug("call getDetailExchRecords({}, {}, {}, {}, {}, {})", new Object[]{sDate, eDate, activity_id, merchantId, pageIndex, pageSize});
		if(merchantId==null || merchantId.trim().length()==0){
			return null;
		}
		StringBuffer sql = new StringBuffer();
		sql.append("select m1.merchant_name as 'shopName',a1.posid as 'posid',mshi.qqUserToken as 'token',mshi.consumeAmount as 'amount',mshi.ts as 'time'  ");
		sql.append(" from Merchant m1,Activitymerchant a1,QQMeishiXaction mshi ");
		sql = concatLimitTime(sql, "mshi.ts", sDate, eDate);
		sql.append(" and a1.activity_id='"+activity_id+"'  ");
		sql.append("and a1.merchant_id = m1.id and mshi.posid = a1.posid and mshi.xactResultCode = '0' ");
		if(!merchantId.equals("all")){
			sql.append("  and m1.id ='"+ merchantId +"' ");
		}
		sql.append(" order by mshi.ts desc, m1.id asc  limit "+pageIndex+","+pageSize);
		logger.trace(sql.toString());
		try {
			SQLQuery q = getSession().createSQLQuery(sql.toString());
			q.addScalar("shopName", Hibernate.STRING);
